...
class LLPTestDatasetConfig(BaseDatasetConfig):
    path_to_data_dir: Path = "${experiment.path_to_data_dir}/tests"
    # dataset_filename: str = "first200"
    dataset_filename: str = "plans_14ac"
...
class GPTModelConfig(BaseModelConfig):
    _target_: str = "llm_sorter.models.gpt_api_model.GPTAPIModel"
    name: str = "gpt3.5-turbo"
...
class LLPProcessorConfig(BaseProcessorConfig):
    ...
    prompt_filename: Optional[str] = "vlm_llp.txt"
    load_prompt_from_file: bool = True


@dataclass
class GPT3ProcessorConfig(BaseProcessorConfig):
    _target_: str = "llm_sorter.processors.GPT3Processor"
    name: str = "gpt3 llp processor"
    run_name: str = "${model.name} ${gen_method.name} ${dataset.dataset_filename}"
    path_to_prompt_dir: Optional[str] = "${hydra:runtime.cwd}/prompts/llp"
...
class HLPProcessorConfig(BaseProcessorConfig):
    ...
    prompt_filename: Optional[str] = "warehouse_hlp.txt"
    load_prompt_from_file: bool = True
    n_examples: int = 4
...
class TestGPT3Config:
    logger: BaseLoggerConfig = field(default_factory=WandbLoggerConfig)
    experiment: BaseExperimentConfig = field(default_factory=LLPExperimentConfig)
    model: BaseModelConfig = field(default_factory=GPTModelConfig)
    metrics: BaseMetricsConfig = field(default_factory=LLPMetricsConfig)
    dataset: BaseDatasetConfig = field(default_factory=LLPTestDatasetConfig)
    gpt_processor: BaseProcessorConfig = field(default_factory=GPT3ProcessorConfig)
    gen_method: BasePlanGenConfig = field(default_factory=FullPlanGenerationConfig)
...
class SorterPlannerConfig:
    logger: BaseLoggerConfig = field(default_factory=WandbLoggerConfig)
    model: BaseModelConfig = field(default_factory=APIModelConfig)
    experiment: BaseExperimentConfig = field(default_factory=SorterPlannerExperimentConfig)
    gen_method: BasePlanGenConfig = field(default_factory=FullPlanGenerationConfig)
    ...
    clean_room_processor: BaseProcessorConfig = field(default_factory=CleanRoomProcessorConfig)
