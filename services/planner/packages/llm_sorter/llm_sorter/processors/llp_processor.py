...
class LLPProcessor(BaseProcessor):
    ...
    def build_system_prompt(
        ...
        if example_tasks and path_to_prompt:
            raise ValueError("Select one way to create prompt: from exaples or from file. Got both.")
            ...
            self._conversation.append_message(self._conversation.roles[1], assistant_output)
        ...
        self._conversation.append_message(self._conversation.roles[0], "How would you {task}?")
    ...
    def _steps_to_text(self, steps: List[LLPStep], add_terminating_string: bool = True) -> str:
        """Transform list of steps (ground true steps) to the one row (plan)"""
        text = ", ".join([f"{step_idx}. {self._step_to_text(step)}" for step_idx, step in enumerate(steps, start=1)])
    ...
    def _text_to_steps(self, task_text: str, cut_one_step: bool = False) -> Union[List[LLPStep], LLPStep, None]:
        if cut_one_step:
...
if __name__ == "__main__":
    logger = WandbLogger()
    dataset = LLPDataset(logger, path_to_data_dir="data/llp/", dataset_filename="first200")
